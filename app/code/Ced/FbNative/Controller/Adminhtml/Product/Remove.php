            ...
            try {
                ...
                foreach ($this->accountCollection as $account) {
                    if ($account->getAccountStatus()) {
                        ...
                        $action->updateAttributes($ids, [$attributeCode => 0], $storeId);
                        try {
                            $action->updateAttributes($ids, ['is_facebook' => 0], $storeId);
                        } catch (\Exception $e) {
                ...
                $this->messageManager->addSuccessMessage(__('Total of %1 record(s) have been Removed from store. Count Include Associated Variants As well.', count($ids)));
            } catch (\Exception $e) {
