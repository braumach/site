...
class Importproducts extends \Magento\Framework\Model\AbstractModel
{
	...
	public function ProductImageGallery($product,$isNewProduct)
    {
    	if(isset($product['gallery']) || isset($product['image']) && isset($product['small_image']) && isset($product['thumbnail'])) 
		{
			...
			if(isset($product["image"]) != "" && substr_count($product["image"],"/") == 0){$product["image"] = "/".$product["image"]; }
			if(isset($product["small_image"]) != "" && substr_count($product["small_image"],"/") == 0){ $product["small_image"] = "/".$product["small_image"]; }
			if(isset($product["thumbnail"]) != "" && substr_count($product["thumbnail"],"/") == 0){ $product["thumbnail"] = "/".$product["thumbnail"]; }
			if(isset($product["swatch_image"]) != "" && substr_count($product["swatch_image"],"/") == 0){ $product["swatch_image"] = "/".$product["swatch_image"]; }
   ...
   protected function addCategories($categories, $storeId)
    {
        ...
        if (empty($this->categoryCache[$storeId])) {
			$collection = $this->Category->getCollection()->setStoreId($storeId)->addAttributeToSelect('name');
