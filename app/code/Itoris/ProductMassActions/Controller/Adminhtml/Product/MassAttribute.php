...
class MassAttribute extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        $attribute = $_attribute = $this->getRequest()->getParam('massAttribute', '');
        ...
        $useDefault = (int)$this->getRequest()->getParam('massAttributeUseDefault', 0);
        ...
        if ($attribute != 'qty' && $attribute != 'stock_status' && $attribute != 'sku') {
        ...
        if (is_array($productIds)) {
            ...
            if ($attribute == 'qty') {
            ...
            } else if ($attribute == 'sku') {
                //applying to each selected product
                $value_orig = $value;
                foreach($productIds as $productId) {
                    $value = $this->parseVariables($value_orig, $productId, $storeId);
                    $con->query("update {$res->getTableName('catalog_product_entity')} set `sku`=".$con->quote($value)." where `entity_id`={$productId}");
                }
            } else {
                ...
                foreach($productIds as $productId) {
                    ...
                    $value = $this->parseVariables($value, $productId, $storeId);
                    ...
                    $newValue = $method == 0 ? $value : round(($method == 1 ? (float)$prevValue['value'] + ($valueType ? floatval($prevValue['value']) / 100 * $value : $value) : (float) $prevValue['value'] - ($valueType ? floatval($prevValue['value']) / 100 * $value : $value)), 2);
                    if ($_attribute == 'price') {
                        $product = $this->_objectManager->create('Magento\Catalog\Model\Product')->setStoreId($storeId)->load($productId);
                        $product->setPrice($useDefault && $storeId ? null : $newValue)->save();
                        continue;
                    }
                    ...
                    if ($useDefault && $storeId) {
                        $con->query("delete from {$db_name} where `attribute_id`={$attribute->getId()} and `store_id`={$storeId} and `{$this->getDataHelper()->_productIndexColumn}`={$productId}");
                    } else if (isset($_prevValue['value_id']) && (int) $_prevValue['value_id']) {
                        $con->query("update {$db_name} set `value`=".$con->quote($newValue)." where `attribute_id`={$attribute->getId()} and `store_id`={$storeId} and `{$this->getDataHelper()->_productIndexColumn}`={$productId}");
                    } else {
                        $con->query("insert into {$db_name} set `value`=".$con->quote($newValue).", `attribute_id`={$attribute->getId()}, `store_id`={$storeId}, `{$this->getDataHelper()->_productIndexColumn}`={$productId}");
                    }
                }
            }
    ...
    public function parseVariables($str, $productId, $storeId){
        preg_match_all( "/{{(.*)}}/U", $str, $matches);
        if (count($matches) && count($matches[1])) {
            $product = $this->_objectManager->create('Magento\Catalog\Model\Product')->setStoreId($storeId)->load($productId);
            foreach($matches[1] as $attr) {
                $attrValue = $product->getData($attr);
                if ($attrValue) $attrValue = $product->getAttributeText($attr);
                if (is_array($attrValue)) $attrValue = implode(', ', $attrValue);
                if (!$attrValue) $attrValue = $product->getData($attr);
                $str = str_replace('{{'.$attr.'}}', $attrValue, $str);
            }
        }
        return $str;
    }
