...
class MassEnable extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if (!is_array($pIds) && !$excluded) {
            ...
            $pIds = $this->_objectManager->create('Ced\TradeMe\Model\Profile')->getCollection()->getAllIds();
        }

        if (!empty($pIds)) {
            try {
                foreach ($pIds as $profileId) {
                    $profile = $this->_objectManager->create('Ced\TradeMe\Model\Profile')->load($profileId);
                ...
                $this->_objectManager->create('Ced\TradeMe\Helper\Logger')->addError('In Mass Enable Profile: ' . $e->getMessage(), ['path' => __METHOD__]);
