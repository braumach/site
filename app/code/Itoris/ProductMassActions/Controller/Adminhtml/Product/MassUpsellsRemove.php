...
class MassUpsellsRemove extends \Magento\Backend\App\Action
{
    /**
     * @return mixed
     */
    public function execute()
    {
        ...
        if (is_array($productIds)) {
            //implementing a faster method of removing upsells through direct SQL
            $res = $this->_objectManager->get('Magento\Framework\App\ResourceConnection');
            $con = $res->getConnection('write');
            
            $con->query("delete from {$res->getTableName('catalog_product_link')} where `product_id` in (".implode(',',$productIds).") and `link_type_id`=".Link::LINK_TYPE_UPSELL." order by `link_id`");
