...
class Process extends \Ced\Integrator\Console\Base
{
    ...
    const PROCESSOR_TYPE_PRODUCT_SEARCH = "search";
    const PROCESSOR_TYPE_REPORT = "report";
    ...
    protected function execute(
        ...
        $result  = '';
        if (!empty($processorType) && $processorType == self::PROCESSOR_TYPE_REPORT) {
        ...
        } elseif (!empty($processorType) && $processorType == self::PROCESSOR_TYPE_PRODUCT_SEARCH) {
            /** @var \Ced\Amazon\Cron\Queue\Search\Processor $processor */
            $processor = $this->om->create(\Ced\Amazon\Cron\Queue\Search\Processor::class);
        } else {
