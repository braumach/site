...
class EbayMultiAccount extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function prepareData($product, $type=null)
    {
        try {
            ...
            if($serviceType == 'Calculated' || true) {
                $weight = $product->getWeight();
                $lbsWeight = floor($weight);
                $ozWeight = $weight - $lbsWeight;
                //$weightarray = explode('.', $weight);
                $item['ShippingPackageDetails']['MeasurementUnit'] = 'Metric';
                $item['ShippingPackageDetails']['WeightMajor'] = ($lbsWeight) ? $lbsWeight : 0;
                $item['ShippingPackageDetails']['WeightMinor'] = ($ozWeight) ? floor($ozWeight * 1000): 0;
                $item['ShippingPackageDetails']['PackageDepth'] = ($product->getData('package_height')) ? $product->getData('package_height') : 0.00;
                $item['ShippingPackageDetails']['PackageLength'] = ($product->getData('package_length')) ? $product->getData('package_length') : 0.00;
                $item['ShippingPackageDetails']['PackageWidth'] = ($product->getData('package_width')) ? $product->getData('package_width') : 0.00;
            }
            ...
            $finalShipService = $shippingType.$shipServices.$internationalshipServices;
            ...
            $ktype = $this->scopeConfig->getValue('ebaymultiaccount_config/product_upload/ktype_value');
            if ($ktype){
                $item['ItemCompatibilityList']['Compatibility'] = 'cedKtype';

                if (!empty($product->getData($ktype))) {
                    $ktypeValues = explode(',', $product->getData($ktype));
                    foreach ($ktypeValues as $value) {
                        $kValues .= '<Compatibility><NameValueList>' . '<Name>KType</Name><Value>'. $value .'</Value>' . '</NameValueList></Compatibility>';
            ...
            if (strpos($finalXml, '<ItemCompatibilityList><Compatibility>cedKtype</Compatibility></ItemCompatibilityList>') !== false) {
                $finalXml = str_replace('<Compatibility>cedKtype</Compatibility>', $kValues, $finalXml);
            }
            if (strpos($finalXml, '<WeightMajor>') !== false) {
                $finalXml = str_replace('<WeightMajor>', '<WeightMajor unit="kg">', $finalXml);
            }
            if (strpos($finalXml, '<WeightMinor>') !== false) {
                $finalXml = str_replace('<WeightMinor>', '<WeightMinor unit="gr">', $finalXml);
            }
            $content = [
                'type' => 'success',
                'data' => $finalXml
