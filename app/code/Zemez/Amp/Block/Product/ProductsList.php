...
namespace Zemez\Amp\Block\Product;
...
class ProductsList extends \Magento\Catalog\Block\Product\AbstractProduct implements BlockInterface, IdentityInterface
{
    ...
    public function getTest() {
        $product_info = $this->getProductCollection()->getItems();
        ...
        foreach ($product_info as $product) {
            $imageUrl = $helperImport->init($product, 'widget-product-grid')->setImageFile($product->getSmallImage())->getUrl();
        ...
        $json_widget_data = array(
            "@context" => "http://schema.org",
            "@type" => "ItemList",
            "url" => $cmsPage->getIdentifier(),
            "numberOfItems" => $this->getProductCollection()->Count(),
        );
        return str_replace('\/', '/', json_encode(array_merge($json_widget_data, $itemListElement)));
