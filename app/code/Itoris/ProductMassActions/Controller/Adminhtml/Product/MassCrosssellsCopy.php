...
class MassCrosssellsCopy extends \Magento\Backend\App\Action
{
    /**
     * @return mixed
     */
    public function execute()
    {
        ...
        if (is_array($productIds)) {
            ...
            $relations = $con->fetchAll("select * from {$res->getTableName('catalog_product_link')} where `product_id`={$fromProductId} and `link_type_id`=".Link::LINK_TYPE_CROSSSELL." order by `link_id`");
            
            if (empty($relations)) {
                $this->messageManager->addError(sprintf(__('Sorry, the product with ID %s has no cross-sell products'), $fromProductId));
                return $this->resultFactory->create(ResultFactory::TYPE_REDIRECT)->setPath('catalog/*/index');
            }
            ...
            foreach($productIds as $productId) {
                ...
                if ($method == 'replace') $con->query("delete from {$res->getTableName('catalog_product_link')} where `product_id`={$productId} and `link_type_id`=".Link::LINK_TYPE_CROSSSELL." order by `link_id`");
                ...
                //updating positions
                $links = (array)$con->fetchCol("select `link_id` from {$res->getTableName('catalog_product_link')} where `product_id`={$productId} and `link_type_id`=".Link::LINK_TYPE_CROSSSELL);
                if (count($links)) {
                    $positionAttributeId = (int)$con->fetchOne("select `product_link_attribute_id` from {$res->getTableName('catalog_product_link_attribute')} where `product_link_attribute_code`='position' and `link_type_id`=".Link::LINK_TYPE_CROSSSELL);
