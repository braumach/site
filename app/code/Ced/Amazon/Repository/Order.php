...
class Order implements \Ced\Amazon\Api\OrderRepositoryInterface
{
    ...
    public function create($accountId, $order = null, \Amazon\Sdk\Api\Order $data = null, $items = [])
    {
        try {
            ...
            $marketplaceOrder->setData(\Ced\Amazon\Model\Order::COLUMN_FULFILLMENT_CHANNEL, $data->getFulfillmentChannel());
            $marketplaceOrder->setData(\Ced\Amazon\Model\Order::COLUMN_SALES_CHANNEL, $data->getSalesChannel());
            $marketplaceOrder->setData(\Ced\Amazon\Model\Order::COLUMN_IS_PRIME, (int)$data->getIsPrime());
            $marketplaceOrder->setData(\Ced\Amazon\Model\Order::COLUMN_IS_BUSINESS, (int)$data->getIsBusinessOrder());
            $marketplaceOrder->setData(\Ced\Amazon\Model\Order::COLUMN_IS_PREMIUM ,(int)$data->getIsPremiumOrder());
