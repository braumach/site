...
class Product extends \CatchSdk\Core\Request
{
    /**
    ...
    public function getCategories($params, $forceFetch = false)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "CatchSdk\\Product\\getCategories() : Errors: " . var_export($e->getMessage(), true)
                );
            }
        }
    ...
    public function getAttributes($params, $forceFetch = false)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "CatchSdk\\Product\\getAttributes() : Errors: " . var_export($e->getMessage(), true)
                );
            }
        }
        return $attributes;
    ...
    /**
     * Create/Update Product on Catch
     * @param $data
     * @return bool|array
     */
    ...
    public function getInventory($skus = [])
    {
        if (!empty($skus) and is_array($skus)) {
            ...
            try {
                ...
                if ($this->debugMode) {
                    $this->logger->addDebug('\CatchSdk\Product\getInventory(): Error: ' . $e->getMessage());
                }
                return false;
            }
        }
    ...
    public function getValueLists($attributeCode, $forceFetch = false, $url = self::GET_ATTRIBUTES_VALUE_LIST)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "CatchSdk\\Product\\getValueLists() : Errors: " . var_export($e->getMessage(), true)
                );
            }
            return $valueLists;
        }
    ...
    public function getOffers($chunkSize = 50, $offset = 0, $url = self::GET_OFFERS)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "CatchSdk\\Product\\getOffers() : Errors: " . var_export($e->getMessage(), true)
                );
            }
            return $offers;
        }
