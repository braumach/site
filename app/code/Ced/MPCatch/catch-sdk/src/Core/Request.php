...
abstract class Request implements \CatchSdk\Core\RequestInterface
{
    /**
     * Debug Logging
     * @var $debugMode
    ...
    /**
     * XML Parser
     * @var \CatchSdk\Core\Generator
     */
    public $xml;

    /**
     * Parser
     * @var \CatchSdk\Core\Parser
     */
    public $parser;
     ...
     */
    public function __construct(\CatchSdk\Core\ConfigInterface $config)
    {
   ...
   /**
    ...
    public function responseParse($response = false, $type = self::FEED_CODE_ITEM_UPDATE, $filePath = '')
    {
        ...
        $feedResponse = array(
            ...
            'feed_type' => $type,
            'feed_status' => 'Catch api failed. Kindly check the api logs.',
            'feed_errors' => "{\"Api Failure\":\"Catch api failed. Kindly check the api logs.\"}",
            'feed_file' => $filePath
        );
        ...
        if ($type && $response) {
            try {
                ...
                if ($this->debugMode) {
                    $this->logger->debug(
                        "CatchSdk\\Core\\Request\\responseParse(): \n Response : \n" . var_export($response, true) .
                        "\n Exception : \n" . var_export($e->getMessage(), true)
                    );
                }
            }
    ...
    public function getFeeds($feedId = null, $subUrl = self::GET_FEEDS_SUB_URL)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    sprintf(
                        "CatchSdk\\Core\\Request\\getFeeds(): \n Response : \n %s \n Exception : %s \n",
                        var_export($response, true),
                        var_export($e->getMessage(), true)
                    )
                );
    ...
    public function validateXml($filePath, $xsdPath, $type = 'lmp-item')
    {
        ...
        if ($result === true) {
            if (($feed->schemaValidate($xsdPath))) {
                ...
                if ($this->debugMode) {
                    $this->logger->debug(
                        "CatchSdk\\Core\\Request\\validateXml(): \n" . "\n Errors : \n" . var_export($errorList, true)
                    );
                }
