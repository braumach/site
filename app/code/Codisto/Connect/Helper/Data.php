...
class Data
{
    ...
    {
file_put_contents($this->getSyncPath('checkRequestHash.log'), "checkRequestHash 1\n", FILE_APPEND);

        if (!isset($server['HTTP_X_NONCE'])) {
file_put_contents($this->getSyncPath('checkRequestHash.log'), "checkRequestHash 2\n", FILE_APPEND);
            return false;
        }

        if (!isset($server['HTTP_X_HASH'])) {
file_put_contents($this->getSyncPath('checkRequestHash.log'), "checkRequestHash 3\n", FILE_APPEND);
            return false;
        }
...
//file_put_contents($this->getSyncPath('checkRequestHash.log'), "checkRequestHash 9: ".$e->getMessage()." on line: ".$e->getLine()." in file: ".$e->getFile()."\n", FILE_APPEND);
