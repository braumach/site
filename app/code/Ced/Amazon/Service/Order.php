...
class Order implements OrderServiceInterface
{
    ...
    private function loadByData($orderIdList = [], $accountId = null, $config = null)
    {
        ...
        $collection = $this->amazonOrderRepository
            ->getByAmazonOrderIds(
                ...
                [\Ced\Amazon\Model\Source\Order\Status::SHIPPED,
                    \Ced\Amazon\Model\Source\Order\Status::FAILED]
            );
        ...
        foreach ($collection as $item) {
            ...
            $orderItems = json_decode($item->getOrderItems(), true);
            if (!empty($orderItems)) {
            ...
            if (!empty($marketplaceOrder) && $create) {
                if (empty($marketplaceOrder->getData(\Ced\Amazon\Model\Order::COLUMN_MAGENTO_ORDER_ID)) &&
                    ...
                    if (in_array($order->getOrderStatus(), [\Ced\Amazon\Model\Source\Order\Status::SHIPPED, \Ced\Amazon\Model\Source\Order\Status::CANCELLED])) {
                        $magentoOrderId = $marketplaceOrder->getData(\Ced\Amazon\Model\Order::COLUMN_MAGENTO_ORDER_ID);
                        if ($magentoOrderId) {
                            $status = false;
