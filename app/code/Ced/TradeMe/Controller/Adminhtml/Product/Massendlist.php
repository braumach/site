...
class Massendlist extends Action
{
    ...
    public function execute()
    {
        ...
        $ids = $this->filter->getCollection($this->catalogCollection->create()->addFieldToFilter($prodStatusAccAttr, 'uploaded'))->getAllIds();
        if (!empty($ids)) {
            ...
            $resultPage->getConfig()->getTitle()->prepend(__('Withdraw Listing From TradeMe'));
            return $resultPage;
        } else {
            $this->messageManager->addErrorMessage(__('No product available to Withdraw.'));
