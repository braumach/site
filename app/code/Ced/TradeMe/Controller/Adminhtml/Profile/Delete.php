...
class Delete extends \Magento\Customer\Controller\Adminhtml\Group
{
    ...
    public function execute()
    {
        ...
        if ($code) {
            $model = $this->_objectManager->create('Ced\TradeMe\Model\Profile')->getCollection()->addFieldToFilter('profile_code', $code);
            // entity type check
            try {
                ...
                $this->_objectManager->create('Ced\TradeMe\Helper\Logger')->addError('In Delete Profile: '.$e->getMessage(), ['path' => __METHOD__]);
                $this->messageManager->addErrorMessage($e->getMessage());
                return $resultRedirect->setPath(
                    'trademe/profile/edit',
                    ['pcode' => $this->getRequest()->getParam('pcode')]
                );
            }
        }
        $this->_redirect('trademe/profile/index');
        return ;
    }
}
