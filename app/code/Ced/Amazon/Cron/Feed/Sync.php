...
use Ced\Amazon\Api\Data\FeedSearchResultsInterface;
...
class Sync
{
    ...
    private function getList($status = \Ced\Amazon\Model\Source\Feed\Status::SUBMITTED, $type = null)
    {
        ...
        $statusFilter->setField(Feed::COLUMN_STATUS)
            ->setConditionType('eq')
            ->setValue($status);
        ...
        if (!empty($type)) {
            ...
            $typeFilter->setField(Feed::COLUMN_TYPE)
                ->setConditionType('in')
                ->setValue($type);
        }
        ...
        /** @var FeedSearchResultsInterface $list */
