...
use Amazon\Sdk\Api\Order\OrderList;
...
class Quote implements QuoteServiceInterface
{
    ...
    public function create($order = null)
    {
        try {
            ...
            /** @var array $name */
            $name = $this->getNameByOrder($order);
            ...
            if ($this->itemAccepted > 0 && count($quoteItems) >= ($this->itemOrdered + $this->bundleItems)) {
                // Assigning address to quote
                $shippingAddress = $this->addAddress($order, $customer, $quote);
     ...
     * @return mixed
     */
    private function addAddress($order, $customer, $quote)
    {
        $shipAddress = [];
        /** @var array $address */
        ...
        if (!empty($address)) {
            ...
            // Magento treats 'Puerto Rico' as State while in Amazon as Country
            if ($countryId == "PR") {
                $countryId = "US";
            ...
            if ($allowedLines >= 3) {
                ...
                $street = $this->getValue('AddressLine1', $address, 'N/A') . " " .
                    $this->getValue('AddressLine2', $address, '') . " " .
                    $this->getValue('AddressLine3', $address, '');
            }
        ...
        if (!empty($this->config->getUseDefaultBilling()) &&
            ...
            try {
                ...
                $this->logger->error(
                    "Default billing email is invalid.",
                    [
                        'path' => __METHOD__,
     ...
     * @return array
     */
    private function getNameByOrder($order)
    {
        //$name = ["N/A", "N/A"];
        $address = $order->getShippingAddress();
