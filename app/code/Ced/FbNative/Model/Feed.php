...
class Feed extends \Magento\AdminNotification\Model\Feed
{
    ...
    const XML_FEED_URL_PATH     = 'system/fbnative/feed_url';
    const XML_FREQUENCY_PATH    = 'system/fbnative/frequency';
    const XML_LAST_UPDATE_PATH  = 'system/fbnative/last_update';
    ...
    public function checkUpdate()
    {
        ...
        $feedXml = $this->getFeedData($this->_objectManager->get('Ced\FbNative\Helper\Feed')->getEnvironmentInformation());
       
        $allowedFeedType = explode(',', $this->_backendConfig->getValue(self::XML_FEED_TYPES));
        
        if ($feedXml && $feedXml->channel && $feedXml->channel->item) {
            $installedModules=$this->_objectManager->get('Ced\FbNative\Helper\Feed')->getCedCommerceExtensions();
            foreach ($feedXml->channel->item as $item) {
               
                if(!isset($installedModules[(string)$item->module])) {
                    continue;
                }
                if($this->_objectManager->get('Ced\FbNative\Helper\Feed')->isAllowedFeedType($item)) {
