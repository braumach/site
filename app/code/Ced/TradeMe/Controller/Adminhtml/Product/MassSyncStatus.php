...
class MassSyncStatus extends Action
{
    ...
    public function execute()
    {
        ...
        $prodStatusAccAttr = $this->multiAccountHelper->getProdListingIdAttrForAcc($accountId);
        $ids = $this->filter->getCollection($this->catalogCollection->create()->addFieldToFilter($prodStatusAccAttr, ['neq' => null]))->getAllIds();
        if (!empty($ids)) {
            ...
            $resultPage->getConfig()->getTitle()->prepend(__('Sync Product(s) Status On TradeMe'));
            return $resultPage;
        } else {
